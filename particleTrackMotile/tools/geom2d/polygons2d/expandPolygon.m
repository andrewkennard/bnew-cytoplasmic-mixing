function [loops,lines,poly2] = expandPolygon(poly, dist, varargin)
...
% EFK: updated to allow for collinear segments
lines = [lines;lines(1,:)];
poly2 = zeros(N, 2);
for i = 1:N
    poly2(i,1:2) = intersectLines(lines(i,:), lines(i+1,:),1e-10);
    if (any(isinf(poly2(i,:))))
        poly2(i,:) = lines(i+1,1:2);
    end
end
