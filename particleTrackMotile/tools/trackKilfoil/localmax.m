function r = localmax(image,sep,field,intminp)
...
%   (more precisely, throws out bottom 1/5 of pixels
    if (intmin < 5) 
        ...
        % get the cutoff value for the top intminp fraction of intensities
    sa = size(a,1)*size(a,2); % EFK: handle nonsquare images
    allvals = reshape(double(a),sa,1);
    allvals = sort(allvals);
    intmin = allvals(floor(sa*(1-intminp)));
end
%intmin
