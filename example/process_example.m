...
storagedirpath = './example/20141223-Zyla_FN_LysoTracker_100nM_Diff_Z_Planes/';
% glob for the name of the directory containing each movie
% %d will be replaced with the movie number
storagedirname = 'M1_*_Cell%d_50ms*';
...
for mc = 1:length(movnums);
    ...
    if (loadsegparams(mc))
        ...
        dirglob = [storagedirpath sprintf(storagedirname,mnum)]
        
        % get list of tif stacks for this movie
        storagedirname = dir(dirglob);
        storagedirname = [storagedirpath storagedirname(1).name '/']
        files = dir([storagedirname '*.ome.tif']);
        files = {files.name};
        
        % get metadata file
        metafile = dir([storagedirname '*metadata.txt']);
